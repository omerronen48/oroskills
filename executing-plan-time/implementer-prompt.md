# Implementer Subagent Prompt Template (executing-plan-time)

Adapted from superpowers:subagent-driven-development. Differences from upstream:

- The worktree path is provided — the implementer does NOT create or change it.
- Pre-queried graphify context is provided — the implementer does NOT call Read/Grep on the wider repo, only on files in its task's manifest entries.
- The TDD-before-commit contract is mandatory: fail-log → pass-log → commit triple is the artifact.
- The implementer may NOT touch any file outside the task's manifest entries.
- The implementer runs ONLY its task's tests, not the full suite.
