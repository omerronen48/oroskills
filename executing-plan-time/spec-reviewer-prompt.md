# Spec Compliance Reviewer Prompt Template (executing-plan-time)

Adapted from superpowers:subagent-driven-development. Differences from upstream:

- The reviewer also verifies the **manifest constraint** (no files touched outside the task's Files list).
- The reviewer also verifies the **TDD artifact** (fail log + pass log + single commit).
- Reads happen in the worktree, not the main checkout.

**Purpose:** Verify the implementer built exactly what was requested, only inside the allowed files, with a real TDD artifact.
