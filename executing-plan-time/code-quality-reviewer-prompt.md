# Code Quality Reviewer Prompt Template (executing-plan-time)

Adapted from superpowers:subagent-driven-development. Differences from upstream:

- Only dispatched AFTER spec compliance review returns PASS.
- Adds a sibling-task check: did this task introduce a symbol that conflicts with files modified by other tasks in the same wave?
- Reads happen in the worktree, with explicit base/head SHAs.

**Purpose:** Verify the implementation is well-built (clean, tested, maintainable) AND does not create latent conflicts with sibling tasks in the same wave.
